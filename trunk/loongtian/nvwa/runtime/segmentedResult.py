...
class BlockChain:
    ...
    def isSingleBlock(self):
        """
        当前BlockChain是否是单一block
        :return:
        """
...
class Block:
    ...
    def __repr__(self):
        return "{<word>:%s, <position>:%s, <isMeta>:%s, <frequncy>:%s)}" % (
        self.word, self.position, str(self.isMeta), str(self.frequncy))


class FilesSegmentResults(dict):
    """
    文章列表的分割结果
    """
    pass
